...
import { profileById, ROUNDHOUSE_PROFILES, type RoundhouseProfile } from '@/lib/profiles';
...
export default function ProfileScreen() {
  ...
  const initialProfile = profileById(profileId);
  ...
  const otherProfiles = useMemo(
    () => ROUNDHOUSE_PROFILES.filter((profile) => profile.id !== selected.id),
    [selected.id],
  );
  ...
  const returnToCommandCenter = () => {
    router.replace({ pathname: '/(tabs)', params: { profileId: selected.id } });
  };
...
function ProfileSwitcher({
  ...
  onSelect: (profile: RoundhouseProfile) => void;
  onClose: () => void;
}) {
  const insets = useSafeAreaInsets();
  return (
